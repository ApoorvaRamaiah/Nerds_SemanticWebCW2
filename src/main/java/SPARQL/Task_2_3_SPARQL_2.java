...
public Task_2_3_SPARQL_2(String input_file, Map<String, Integer> column_index) throws IOException {
...
protected void CovertCSVToRDF(boolean useExternalURI) throws JsonProcessingException, IOException, URISyntaxException {
		    ...
		    mappingToCreateLiteralTriple(column_index.get("menu_item"), column_index.get("item_value"), cw_ns_str + "amount", XSDDatatype.XSDinteger);
		    ...
		    mappingToCreateLiteralTriple(column_index.get("item_value"), column_index.get("item_value"), cw_ns_str + "amount", XSDDatatype.XSDinteger);
...
public void performSPARQLQuery(Model model, String file_query_out) {
   ...
   String queryStr = 
		   "PREFIX cw: <http://www.semanticweb.org/city/in3067-inm713/2024/restaurants#>\n" +
	                ...
	                "SELECT DISTINCT ?restaurant ?menu_item ?state ?postcode ?item_value WHERE {\n" +
	                ...
	                "    FILTER (?postcode = '63105' && xsd:double(?item_value) = 15)\n" +
	                "}\nORDER BY (?restaurant)";
	...
	try {
		...
		while( res.hasNext()) {
			...
			writer.writeLine(restaurant.toString()+","+state.toString()+","+menu_item.toString()+","+item_value.toString()+","+postcode.toString());
			//writer.writeLine(restaurant.toString()+","+item_value.toString());
...
public static void main(String[] args) {
	...
	try {
		Task_2_3_SPARQL_2 solution = new Task_2_3_SPARQL_2(file, column_index);
